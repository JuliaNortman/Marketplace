...
public class OrderServiceImpl implements OrderService {
    ...
    public List<OffsetDateTime> getFreeSlots() {
        ...
        final int START_WORKING_DAY_HOUR = 7;
        final int END_WORKING_DAY_HOUR = 18;
        ...
        while (endDay.isBefore(OffsetDateTime.now().plusWeeks(2).withMinute(0).withSecond(0).withNano(0))) {
            while (endDay.getHour() <= END_WORKING_DAY_HOUR) {
            ...
            endDay = endDay.plusDays(1).withHour(START_WORKING_DAY_HOUR).withMinute(0).withSecond(0).withNano(0);
        }
        return freeSlots;
    }
