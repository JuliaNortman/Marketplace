...
(
    ...
    NULL
    CONSTRAINT
    shopping_cart_item_goods_id_fk
    REFERENCES
    goods,
    quantity
    INTEGER
...
CREATE
UNIQUE INDEX IF NOT EXISTS ux_role_role  ON role (role);
CREATE
INDEX credentials_email_hash_index ON credentials USING hash(email);

CREATE
EXTENSION pg_trgm;
CREATE
INDEX person_full_name_index ON person using gin ((name || ' ' || surname) gin_trgm_ops);
CREATE
INDEX products_name_index ON product USING gin (name gin_trgm_ops);
CREATE
INDEX orderdetails_deliverytime_btree_index ON order_details USING btree(delivery_time);
